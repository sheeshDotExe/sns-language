...
void addBuiltin(struct Builtins *builtins, unsigned int index, char *name, unsigned int inputs, struct Var **inputVars, struct Var* returnValue, struct Var *(*function)(struct Param *params, struct State *state), struct ProcessState* processState)
{
	...
	builtins->functions[index].originalParams = copyParam(builtins->functions[index].params, processState);
}
...
struct Builtins* createBuiltins(struct ProcessState* processState)
{
	...
	types[0] = String_c;
	inputVars[0] = generateVar(&types, 1, "path", "", NULL, processState);
	returnValue = generateVar(NULL, 0, "return", "", NULL, processState);
	addBuiltin(builtins, 0, strdup("route"), 1, inputVars, returnValue, &route, processState);

	inputVars = (struct Var**)malloc(sizeof(struct Var*) * 1);
	types[0] = String_c;
	inputVars[0] = generateVar(&types, 1, "path", "", NULL, processState);
	returnValue = generateVar(NULL, 0, "return", "", NULL, processState);
	addBuiltin(builtins, 1, strdup("route_static"), 1, inputVars, returnValue, &route_static, processState);
	...
	inputVars[0] = generateVar(&types, 1, "path", "", NULL, processState);
	returnValue = generateVar(NULL, 0, "return", "", NULL, processState);
	addBuiltin(builtins, 2, strdup("html"), 1, inputVars, returnValue, &html, processState);
	...
	types[1] = Int_c;
	types[2] = Float_c;
	inputVars[0] = generateVar(&types, 3, "condition", "", NULL, processState);
	returnValue = generateVar(NULL, 0, "return", "", NULL, processState);
	addBuiltin(builtins, 3, strdup("if"), 1, inputVars, returnValue, &if_implementation, processState);

	inputVars = (struct Var**)malloc(0);
	returnValue = generateVar(NULL, 0, "return", "", NULL, processState);
	addBuiltin(builtins, 4, strdup("else"), 0, inputVars, returnValue, &else_implementation, processState);
